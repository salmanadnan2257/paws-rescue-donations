...
- **Writing the exhaustive project documentation honestly.** Documenting a smaller, more contained project like this one honestly still meant treating the source as the only truth: several design weaknesses the README already flags itself (a missing Donation-to-User foreign key, an unvalidated currency field) are quoted from the project's own self-assessment rather than presented as newly discovered issues.
